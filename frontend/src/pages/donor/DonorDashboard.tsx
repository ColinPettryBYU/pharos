...
import { mockDonations, mockAllocations, mockSafehouses } from "@/lib/mock-data";
...
export default function DonorDashboard() {
  const { user } = useAuth();
  // Use supporter_id 1 for demo
  const supporterId = user?.linkedSupporterId ?? 1;
  const donations = mockDonations.filter((d) => d.supporter_id === supporterId);
  const totalDonated = donations.reduce((s, d) => s + d.amount, 0);
  const allocations = mockAllocations.filter((a) => donations.some((d) => d.donation_id === a.donation_id));

  const allocationByArea = allocations.reduce<Record<string, number>>((acc, a) => {
    const name = mockSafehouses.find((s) => s.safehouse_id === a.safehouse_id)?.name ?? `Safehouse ${a.safehouse_id}`;
    acc[name] = (acc[name] || 0) + a.amount_allocated;
    return acc;
  }, {});
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      >
        <h1 className="text-3xl font-bold tracking-tight">
          Thank you, {user?.displayName || "Valued Supporter"}.
        </h1>
        <p className="mt-1 text-lg text-muted-foreground">
          Here's the impact of your generosity.
        </p>
      </motion.div>
      ...
      >
        ...
        <motion.div variants={item}>
          <StatCard title="Donations" value={donations.length} icon={Heart} />
        </motion.div>
        <motion.div variants={item}>
          <StatCard title="Supporter Since" value={2023} icon={Calendar} />
        </motion.div>
      </motion.div>
        ...
        <Card>
          ...
          <CardContent>
            {pieData.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-8">No allocation data yet.</p>
            )}
          </CardContent>
        </Card>
      </motion.div>
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground max-w-lg mx-auto">
              Your contributions have supported {new Set(allocations.map((a) => a.safehouse_id)).size} safehouses,
              helping provide education (avg 78% progress), healthcare (avg 4.2/5 health score),
              and counseling services to girls across the Philippines.
            </p>
          </CardContent>
        </Card>
      </motion.div>
      ...
      >
        ...
        <DataTableWrapper columns={donationCols} data={donations} />
      </motion.div>
